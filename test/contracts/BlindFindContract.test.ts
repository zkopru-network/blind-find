...
describe("Blind Find Contract", function() {
  ...
  it('should succeed when admin update the merkle root', async () => {
    ...
    expect(merkleRoots.length).to.eql(2);
    expect(merkleRoots[0]).to.eql(firstMerkleRoot);
    expect(merkleRoots[1]).to.eql(secondMerkleRoot);
  });
